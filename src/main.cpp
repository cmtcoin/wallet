...
#if defined(NDEBUG)
# error "Comet cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb3fe3b223df0adc3047ffa9517f7e061e104adf083efe609c2bc136299ec5403");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Comet: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Comet Signed Message:\n";
...
{
    // Comet: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 97000 blocks
    nSubsidy >>= (nHeight / 97000);
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Comet: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Comet: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1462823105, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 09 05 2016 A Canadian Boom Town Is Left in Ashes";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0426a37df518d6ce6b502853178569d2bb54a36445ae7a28eb6952707f5d46ebe8de530ec50511fd6a5a1deb3fc8a27780e846395f790c25d5a3974e11c10d158b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1462823105;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3982555;
        ...
        assert(block.hashMerkleRoot == uint256("0x7da5a43cb5c181d8a44661889f7023242bcc19ee13e1d4b7d7d093403849545e"));
...
unsigned char pchMessageStart[4] = { 0x79, 0xc0, 0x7c, 0x31 }; // Comet: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CometMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CometMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CometMiner(CWallet *pwallet)
{
    printf("CometMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("comet-miner");
    ...
    try { loop {
        ...
        printf("Running CometMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CometMiner, pwallet));
}
